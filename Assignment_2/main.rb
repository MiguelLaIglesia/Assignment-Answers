...
File.open(output_report_file, 'w') do |file|
  ...
  Networks.all_networks.each_with_index do |network, idx|
    ...
    file.puts "KEGG annotations for all proteins in this network: #{network.network_KEGGs}"
    file.puts "GO annotations for all proteins in this network: #{network.network_GOs}"
    end
  end 
